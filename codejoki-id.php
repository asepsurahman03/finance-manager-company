...
$company_filter = "Codejoki.id"; // Hanya tampilkan transaksi dari Codejoki.id
...
// Validasi filter jumlah
if ($amount_filter && !in_array($amount_filter, ['net_amount', 'worker_amount'])) {
    die("Filter jumlah tidak valid.");
}

// Query dasar hanya untuk Codejoki.id
$query = "SELECT * FROM transactions WHERE user_id = :user_id AND company = :company";
$params = ['user_id' => $user_id, 'company' => $company_filter];
...
// Data bulanan hanya untuk Codejoki.id
try {
    $month_query = "SELECT YEAR(date) AS year, MONTH(date) AS month, 
                        SUM(amount) AS total, 
                        SUM(net_amount) AS total_net, 
                        SUM(worker_amount) AS total_worker 
                FROM transactions 
    ...
    $month_stmt->execute(['user_id' => $user_id, 'company' => $company_filter]);
...
$perPage = 2; // Menampilkan 10 transaksi per halaman
$totalTransactions = count($transactions);
$totalPages = ceil($totalTransactions / $perPage);
$page = isset($_GET['page']) ? (int)$_GET['page'] : 1;
$start = ($page - 1) * $perPage;

// Query dengan limitasi 10 transaksi per halaman
$query = "SELECT * FROM transactions WHERE user_id = :user_id AND company = :company";
...
try {
    ...
    $stmt->bindValue(':company', $company_filter, PDO::PARAM_STR);
    $stmt->bindValue(':start', $start, PDO::PARAM_INT);
    $stmt->bindValue(':perPage', $perPage, PDO::PARAM_INT);

    if ($type_filter) {
        $stmt->bindValue(':type', $type_filter, PDO::PARAM_STR);
    }
...
<head>
    ...
    <title>Pendapatan & Pengeluaran Codejoki.id</title>
</head>
<body class="bg-gray-100">
    <div class="flex flex-col lg:flex-row h-full lg:h-screen">
        ...
        <div class="flex-1 p-4 md:p-6 bg-gray-100">
            <h1 class="text-3xl font-bold mb-6">Pendapatan & Pengeluaran Codejoki.id</h1>

            <!-- Filter -->
            <div class="mb-6 flex flex-wrap gap-4">
                <a href="codejoki-id.php" class="px-6 py-3 bg-gray-500 text-white rounded-lg hover:bg-gray-600 transition duration-300">Semua</a>
...
<script>
    ...
    function printTransactions() {
        ...
        const reportTitle = `    <h3 style="text-align: center; font-size: 1.5em; font-weight: bold; margin-bottom: 15px;">Laporan Pendapatan & Pengeluaran Codejoki.id - Pada Bulan ${monthName} ${year}</h3>`;
